...
void evaluation(vector<vector<Cell>> &cells, int currentTick)
{
    ...
    if (currentTick == 0)
    {
        name1 << "currentTick,NO_CHANGE,DIE,QUARANTINE,RECOVER,NO_INFECTIOUS,SINFECTED \n";
        name2 << "currentTick,NO_CHANGE,DIE,QUARANTINE,RECOVER,NO_INFECTIOUS,SINFECTED \n";
    }
    ...
    if (currentTick >= 0)
    {
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                if (cells[i][j].state != NO_POPULATED)
                {
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            ...
            case DIE:
                ...
                break;
            case NO_POPULATED:
                glColor3f(0.611764705882353, 0.6274509803921569, 0.6509803921568628);
                break;
            }
            glRectd(i, j, i + 1, j + 1);
        }
